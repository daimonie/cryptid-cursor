...
def select_top_moves(
    generator, q_matrix, moves_with_states, hint, n=10, learning_rate=0.1
):
    ...
    for move in moves_with_states:
        ...
        avg_q_value = sum(
            get_q_value(q_matrix, move[:2], state, hint) for state in states
        ) / len(states)
...
def select_top_cube_moves(
    generator, q_matrix, cube_moves, hint, n=10, learning_rate=0.1
):
    if generator.random() < learning_rate:
...
def generate_all_hints() -> Dict[str, List[str]]:
    ...
    categories = {
        ...
        "within_one": [
            *[
                (f"is_{terrain}", f"neighbor_is_{terrain}")
                for terrain in get_terrain_types()
            ],
            *[
                (f"is_{animal.lower()}", f"neighbor_is_{animal.lower()}")
        ...
        "within_two": [
            *[
                (
                    f"is_{animal}",
                    f"neighbor_is_{animal}",
                    f"neighbor_neighbor_is_{animal}",
                )
                for animal in get_all_animals()
            ],
            *[
                (
                    f"{structure}",
                    f"neighbor_{structure}",
                    f"neighbor_neighbor_{structure}",
                )
        ...
        "within_three": [
            *[
                (
                    f"{color}",
                    f"neighbor_{color}",
                    f"neighbor_neighbor_{color}",
                    f"neighbor_neighbor_neighbor_{color}",
                )
                for color in colors
            ]
        ],
    ...
    for key in categories:
        categories[key] = [
            (item,) if isinstance(item, str) else item for item in categories[key]
        ]
...
def verify_map_attributes(G):
    ...
    for category in generate_all_hints().values():
        for hint in category:
            hint_attributes.update(
                attr for attr in hint if not attr.startswith("neighbor_")
            )
...
def find_available_placements(G, player_hint):
    ...
    for node in G.nodes():
        ...
        no_own_pieces = not any(
            G.nodes[node].get(f"{pt}_player{i}", False)
            for pt in ["cube", "disc"]
            for i in range(1, 4)
        )

        # For cubes, also check if there are no other players' cubes
        no_other_cubes = not any(
            G.nodes[node].get(f"cube_player{i}", False) for i in range(1, 4)
        )
...
def update_q_matrix(q_matrix, moves, final_state, player_won, **kwargs):
    ...
    for i in range(len(moves) - 1, -1, -1):
        ...
        if i == len(moves) - 1:
            ...
            reward = (
                move_penalty * (1 - gamma ** (T - t)) / (1 - gamma)
                + (gamma ** (T - t)) * R
            )
        ...
        new_q = current_q + learning_rate * (
            reward + discount_factor * next_q_max - current_q
        )
