...
def serialize_graph(graph: nx.Graph, hints: Dict[str, List[str]] = None) -> str:
    ...
    graph_data = {
        "nodes": {
            row_col_to_node_id(*node, "AB"): dict(data)
            for node, data in graph.nodes(data=True)
        },
        "edges": [
            (row_col_to_node_id(*u, "AB"), row_col_to_node_id(*v, "AB"))
            for u, v in graph.edges()
        ],
    }

    if hints:
        hints_json = {f"player{i+1}": hints[i].tolist() for i in range(3)}
...
def parse_code_to_graph(graph_hash: str) -> nx.Graph:
    ...
    try:
        ...
        raise FileNotFoundError(
            "'/opt/container/output/{graph_hash} file not found in /opt/container/output/"
        )

    graph = nx.Graph()
    graph.add_nodes_from(
        (node_id_to_row_col(node), attrs) for node, attrs in graph_data["nodes"].items()
    )
    graph.add_edges_from(
        (node_id_to_row_col(u), node_id_to_row_col(v)) for u, v in graph_data["edges"]
    )
...
def filter_nodes_by_attributes(graph: nx.Graph, *args) -> list:
    ...
    def node_matches_conditions(node_attrs):
        return all(
            node_attrs.get(attr) == value for attr, value in attr_conditions.items()
        )

    return [
        node for node, attrs in graph.nodes(data=True) if node_matches_conditions(attrs)
    ]
